      ...
      ical.fromURL(url, {}, function(err, data) {
        ...
        data = _.filter(data, (event) => {
          ...
          if (event.rrule) {
            ...
            var matched = _.find(between, date => moment(date).isBetween(from, to, null, '[]'));
            if (matched) {
          ...
          if (!event.end) {
            return moment(event.start).isBetween(from, to, null, '[]');
          }
          ...
          // 종일 일정 or 날짜가 바뀌는 일정
          if (diffDay > 0) {
            return moment(event.start).isBetween(from, to, null, '[]') || moment(event.end).isBetween(from, to, null, '[)');
          } else { // 하루내 일정
            return moment(event.start).isBetween(from, to, null, '[]') || moment(event.end).isBetween(from, to, null, '[]');
          }
        });
