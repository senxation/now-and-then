...
const getList = (from) => {
  ...
  const options = {
    ...
    fields: 'nextPageToken, kind, files(contentHints/thumbnail,hasThumbnail,id,imageMediaMetadata,kind,mimeType,name,fileExtension,ownedByMe,properties,size,thumbnailLink,thumbnailVersion,videoMediaMetadata,webContentLink)'
  };
  ...
  const _getList = (nextPageToken) => {
    ...
    return promise.then((nextPageToken) => {
      if (nextPageToken) {
        ...
        isPending = false;
        return _.sortBy(list, item => moment(item.modifiedTime).toDate());
      }
    }, (err) => {
      console.log('error', err)
    });
...
const watchModified = (list) => {
  const promise = new Promise((resolve, reject) => {
    drive.files.list({
      ...
      if (list.length && _.first(list).id === file.id) {
